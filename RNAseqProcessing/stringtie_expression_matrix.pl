...
unless($expression_metric && $result_dirs && $transcript_matrix_file && $gene_matrix_file){
    ...
    print "Usage:  ./stringtie_expression_matrix.pl --expression_metric <metric>  --result_dirs='dirs' --transcript_matrix_file= <filename>.tsv --gene_matrix_file= <filename>.tsv\n\n";
    exit();
}
